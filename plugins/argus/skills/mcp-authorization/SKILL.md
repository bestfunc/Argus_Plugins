...
- `read_file` / `grep_file` / `download_file` / `upload_*` / `file_search` / `archive_file` / `unarchive_file`：`path_deny`（fnmatch 黑名单，如 `*.env` / `*.key`；archive 也检查 sources/dest，unarchive 检查 src/dest_dir）
