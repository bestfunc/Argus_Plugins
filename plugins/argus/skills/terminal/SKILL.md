...
- **超时要看 `process_state` 分两种情况处理**：
  ...
  - `unknown`：Server 没等到回执，进程状态未知，重跑前先用 `tasklist` / `ps` 核实，
    否则会起第二个实例
- **cmd 的 `%ERRORLEVEL%` 在整行读取时就展开了，不是执行到那里才求值**：
  `cmd /c "prog > out.txt & echo rc=%ERRORLEVEL%"` 打出来的是 **prog 跑之前**的值，
  跟 prog 的成败无关——这是个看起来正常的错误答案，据此下结论会作废。
  要真实退出码就拆成独立的一次调用，或在批处理里 `setlocal enabledelayedexpansion` + `!ERRORLEVEL!`。
  > 顺带一个可用判据：命令若**不返回**，它后面那一行根本不会执行，
  > 于是「那个 rc 文件存不存在」本身就是一个读数。
- **被测对象是"会不会常驻不退"时，别用 `run_command` 直接跑它**：超时会 `killed_by_agent`，
  等于用会杀掉被测对象的方式去问"它是不是还活着"，永远问不出来。
  改成后台启动 + 输出重定向，再轮询文件。
- **带轮询的探针要把轮询上限压到远小于 timeout**：轮询 60s 加上 `Add-Type` 的编译时间
  一起超过上限，整条命令超时，输出全丢（只剩 `output_log` 可捞）
