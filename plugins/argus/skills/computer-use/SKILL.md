...
name: computer-use
display_name: 远程操控
description: 通过 Argus Remote Computer Use 远程操控宿主机 GUI——截图、鼠标点击、键盘输入、剪贴板读写、UI 元素识别。注意所有 click/key/type_text 均为 L3，每步都要邮箱审批，执行前务必与用户确认整体方案。
user-invocable: true
...
| `set_clipboard(text)` | 写入剪贴板（只读系统状态变化，L1） |
| `scroll(x, y, delta)` | 滚轮（L1） |

### 🟡 L2 感知类（首次授权 + 15 分钟快路径）

| 工具 | 说明 |
|------|------|
| `screenshot` | 全屏缩略图或指定 region 高清截图（屏幕内容可能含敏感信息） |
...
> ⚠️ **重要**：点鼠标和键盘是 L3，意味着**每次 click/key/type_text 都要一次邮箱验证码**。
> Computer Use 场景下这会非常慢。做法是：
>
> 1. **先规划**——用 L1/L2 工具（`get_ui_elements`、`screenshot`）摸清界面
> 2. **跟用户说清楚整体步骤**——例如"我会依次点 A、输入 B、按 Enter，共 3 次审批"
> 3. **连续执行**——每步调用都带 `_approval_reason`，用户连续 3 次验证码就完成
> 4. 避免无脑试探；每次 click 都有成本
...
```
wait_stable()      → 等动画结束
screenshot(region) → 确认结果（15 分钟内免验证码）
```
...
2. set_clipboard()   ← L1 写剪贴板（可以用 ctrl+v 代替 type_text，但 ctrl+v 是 L3）
3. get_clipboard()   ← L2 读剪贴板（15 分钟快路径）
4. screenshot(region)← L2 局部截图
5. screenshot()      ← L2 全屏缩略图
6. click/key/...     ← L3，每次审批
```
